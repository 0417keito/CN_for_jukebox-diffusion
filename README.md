...
This repository is for adapting ControlNet to Jukebox-diffusion. However, there are several issues. First, the optimization of input_hint_block and zero_conv is required. The diffusion model used in Jukebox-diffusion, inspired by stable diffusion, is different from the diffusion models used in AudioLDM or Tango, necessitating special adjustments for ControlNet. The optimization of input_hint_block and zero_conv is crucial. Also, the sampling method used in Jukebox-diffusion is unique, requiring additional innovation.
