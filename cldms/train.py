import os
import argparse
from jbdiff.utils import read_yaml_file, parse_diff_conf, ExceptionCallback, DemoCallback, JBDiffusion
from cldm_data import make_jb
import torch as t
import pytorch_lightning as pl
from cldm import Control_JBDiffusion

#----------------------------------------------------------------------------

# Change config file to change hyperparams
CONFIG_FILE = '/home/keito/jukebox-diffusion/cldms/configs/jbdiff-control.yaml'
...
def run(*args, **kwargs):
  ...
  control_dir = kwargs['control_data']
  ...
  vqvae, dataloader, hps = make_jb(audio_dir, control_dir, level, batch_size, base_tokens, context_mult, aug_shift, num_workers)
  ...
  # Instantiate model
  diffusion_model = Control_JBDiffusion(vqvae=vqvae, level=level, diffusion_kwargs=diffusion_conf)
...
def main():
  ...
  parser.add_argument('--control-data', help='Location of control data, MAKE SURE all files are .wav format and the same sample rate', required=True, metavar='DIR', type=_path_exists)
